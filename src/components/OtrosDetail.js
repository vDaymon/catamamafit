...
const OtrosDetail = ({ producto = {} }) => {
  ...
  if (!producto) return <div className="pt-20 text-center">Producto no encontrado</div>;
  ...
  return (
    <div className="min-h-screen py-16 px-4 relative" style={{
      ...
      <motion.div
        ref={ref}
        className="max-w-4xl mx-auto bg-white shadow-2xl p-8 md:p-12"
      ...
      >
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
          <motion.div
            className="flex-shrink-0 w-full md:w-80 h-64 md:h-80 overflow-hidden shadow-lg"
            initial={{ opacity: 0, x: -60 }}
            animate={inView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.8, type: 'spring' }}
          >
            <img
              src={producto.image || 'https://via.placeholder.com/320'}
              alt={producto.title || 'Guía'}
          ...
          <div className="flex-grow text-center md:text-left">
            ...
            >
              {producto.title || 'Título de la Guía'}
            </motion.h1>
            <motion.p
            ...
            >
              {producto.description || 'Aquí va una descripción detallada de la guía, sus beneficios, lo que incluye y a quién está dirigido. ¡Anímate a transformarte!'}
            </motion.p>
            <motion.div
              className="mb-8 bg-gray-900 shadow-inner p-6 text-white"
              initial={{ opacity: 0, y: 30 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            >
              ...
              <ul className="list-disc list-inside text-gray-100 space-y-2 text-left mx-auto md:mx-0 max-w-md">
                {producto.includes && producto.includes.length > 0 ? (
                  producto.includes.map((item, index) => (
            ...
            >
              {producto.price ? `${producto.price}` : 'Precio: Consultar'}
            </motion.p>
            <motion.a
              href={producto.link || "https://wa.me/5491112345678"}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block px-8 py-3 bg-pink-600 text-white text-lg font-semibold shadow-lg hover:bg-pink-400 transition-all duration-300 mt-2"
              aria-label="WhatsApp"
        ...
        {/* FAQ Section */}
        <motion.div
          className="mt-12 bg-pink-50 shadow-inner p-6"
        ...
        >
          ...
          <ul className="space-y-3 text-gray-700 text-base">
            {producto.faq && producto.faq.length > 0 ? (
              producto.faq.map((item, idx) => (
                <li key={idx}><strong>{item.q}</strong> {item.a}</li>
              ))
            ) : (
              <li>No hay preguntas frecuentes para esta guía.</li>
            )}
          </ul>
          <div className="mt-6 text-center">
            <motion.a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block px-8 py-3 bg-pink-600 text-white text-lg font-semibold shadow-lg hover:bg-pink-700 transition-all duration-300 mt-2"
              aria-label="WhatsApp"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
